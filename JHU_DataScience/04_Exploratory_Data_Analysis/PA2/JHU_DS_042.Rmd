---
title: "JHU DS Exploratory Analysis PA2"
author: "Colin Chen"
date: "3/8/2022"
...
The purpose of to analyze the EPI National Emissions Invetory's historical data for tracking the emissions of this pollutant into the atmosphere. </br>
...
```{R Plot 1 Setup, message = FALSE, warning = FALSE}
...
dev.off()
```
```{R Plot 2 Setup, warning = FALSE, message = FALSE}
...
```{R Plot 3 Setup, warning = FALSE, message = FALSE}
...
```{R Plot 4 Setup, warning = FALSE, message = FALSE}
...
```{R Plot 5 Setup, warning = FALSE, message = FALSE}
...
```{R Plot 6 Setup, warning = FALSE, message = FALSE}
# Subsetting vehicle related NEI data on each level columns in SCC
vehRel <- grepl("vehicle", SCC$SCC.Level.Two, ignore.case=TRUE)
vehSCC <- SCC[vehRel, SCC]
