...
```{r}
Total_S[, .(StepAvg = (steps, na.rm = TRUE), StepMed = median(steps, na.rm = TRUE))]
```
...
fwrite(x = activity, file = "data/fixed.csv", quote = FALSE)
```
...
Total_S2 <- acitvity[, c(lapply(.SD, sum)), .SDcols = c("steps"), by = .(date)]

## mean & median total number of steps taken per day based on updated data
Total_S2[, .(Mean_Steps = mean(steps), Median_Steps = median(steps))]
