---
title: "Advanced Data Visualization with R: Week 1"
...
library(GGally)
scatter_color <- function(data, mapping){
  ggplot(data = data, mapping = mapping) +
    geom_jitter(color="orange")
}

density_color <- function(data, mapping){
  ggplot(data = data, mapping = mapping) +
  geom_density(alpha = 1, fill = "blue")  
}

ggpairs(data = dat2, lower = list(continuous = scatter_color), diag = list(continuous = density_color))
...
cor3 <- cor(dat3, use = "complete.obs")

ggcorrplot(cor3, type = "lower", title = "Correlations", ggtheme = theme_light())
```
...
ggplot(data = dat4, aes(x = var1, y = reorder(names, var1)))+
  geom_point(size = 5) +
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.major.y = element_line(linetype = "dashed", color = "red"))+
  labs(x = "Variable1", y = NULL)
...
ggplot(data = books_checked_out, aes(x = Time, y = Total, fill = Genre)) + 
  geom_area()+
...
ggplot(data = books_checked_out2, aes(y = reorder(Genre, Time1),x = Time1, xend = Time5)) + 
  geom_dumbbell(
    colour_x = "red",
    colour_xend = "purple",
    size_x = 3,
